...
import { SalesStage, SalesInputMode, SalesAssistRequest, SalesScript } from '@/types/platform'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Headphones, FileText, Users, Sparkles, Copy, ArrowRight } from 'lucide-react'
...
function AssistentePanel() {
  ...
  const handleGenerate = async () => {
    ...
    const result = {
      ...
      avoid: buildAvoid(stage),
      nextStep: buildNextStep(stage),
    }
...
function buildScript(stage: SalesStage, situation: string, base: any): string {
  ...
  switch (stage) {
    case 'prospeccao':
      return `Oi [Nome], vi seu conteúdo sobre [tema] e faz muito sentido. Trabalho com ${base.service || 'especialistas'} entregando ${result} através de ${diff}. Te interessa um diagnóstico rápido de 15min?`
    case 'qualificacao':
      return `Antes de agendar, duas perguntas rápidas: 1) qual seu maior desafio hoje com [dor]? 2) já investiu em solução antes? Assim eu te direciono direito.`
    case 'reuniao':
      return `Baseado no que você compartilhou, o gargalo é [X]. Nosso foco seria ${result} em até 90 dias com ${diff}. Faz sentido pra você?`
    case 'objecao':
      return `Entendo perfeitamente. Me explica uma coisa: o que te faz hesitar — é o investimento, o timing ou a certeza de que vai funcionar pra você?`
    case 'follow_up':
      return `Oi [Nome], passando para saber se você teve chance de pensar. Fico à disposição para tirar qualquer dúvida restante.`
    case 'fechamento':
      return `Então, pelo que conversamos, faz sentido avançar. Você prefere iniciar hoje ou amanhã? Assim já garanto seu lugar na agenda.`
  }
}
function buildAvoid(stage: SalesStage): string[] {
  const map: Record<SalesStage, string[]> = {
    ...
    objecao: ['Não discordar do lead', 'Não justificar demais', 'Não baixar preço de cara'],
    follow_up: ['Não cobrar resposta', 'Não sumir por dias', 'Não repetir a mesma mensagem'],
    fechamento: [
...
function buildNextStep(stage: SalesStage): string {
  const map: Record<SalesStage, string> = {
        ...
        {current ? (
          <div className="rounded-2xl bg-[#14141C] border border-white/5 p-5 space-y-3 sticky top-4">
            ...
            {current.variations.map((v, i) => (
              <div key={i} className="rounded-lg bg-[#0e0e15]/60 border border-white/5 p-3">
                ...
                <p className="text-xs text-slate-200 whitespace-pre-wrap">{v}</p>
              </div>
            ))}
          </div>
        ) : (
      ...
      {profileCaptures.length === 0 ? (
        ...
        <div className="space-y-3">
          {profileCaptures.map((p) => (
            <div key={p.id} className="rounded-xl bg-[#14141C] border border-white/5 p-4 space-y-2">
              <div className="flex items-center justify-between">
